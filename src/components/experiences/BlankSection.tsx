...
import { BlankSectionDisplayTemplate } from '@/display-templates/experience/BlankSectionDisplayTemplate';
...
export default function BlankSection({ content, displaySettings }: BlankSectionProps) {
  ...
  return (
    <section
      className={`vb:grid relative w-full ${sectionSpacing} ${colorScheme} overflow-visible px-4 md:px-6 lg:px-8`}
      {...pa(content)}
    >
...
function Row({
  ...
  displaySettings,
  colGap,
}: StructureContainerProps & { colGap: string }) {
  ...
  const verticalAlignment =
    verticalAlignmentMap[(displaySettings?.verticalAlignment as string) ?? 'start'];
  return (
    <div
...
function Column({
  ...
  displaySettings,
  elemGap,
}: StructureContainerProps & { elemGap: string }) {
  const { pa } = getPreviewUtils(node);
