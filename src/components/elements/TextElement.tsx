...
import { TextElementCT } from '@/content-types/elements/TextElement';
import { TextElementDisplayTemplate } from '@/display-templates/elements/TextElementDisplayTemplate';
...
export default function TextElement({ content, displaySettings }: Props) {
  ...
  switch (headingLevel) {
    case 'h1':
      return (
        <h1 className={`text-4xl font-bold text-foreground ${baseClasses}`} {...pa('text')}>
          {content.text}
        </h1>
      );
    case 'h2':
      return (
        <h2 className={`text-3xl font-bold text-foreground ${baseClasses}`} {...pa('text')}>
          {content.text}
        </h2>
      );
    case 'h3':
      return (
        <h3 className={`text-2xl font-semibold text-foreground ${baseClasses}`} {...pa('text')}>
          {content.text}
        </h3>
      );
    case 'h4':
    ...
    case 'h5':
      return (
        <h5 className={`text-lg font-medium text-muted-foreground ${baseClasses}`} {...pa('text')}>
          {content.text}
        </h5>
      );
    default:
